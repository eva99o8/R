...
cps = c(123, 141, 120, 145, 142, 144, 152, 157, 134, 139, 144, 128, 130)
#cps = c(135, 141, 120, 145, 142, 120, 152, 150, 145, 139, 144, 130, 150) # cps fixed for simulation


list_sim_Y_cp = gen_Y_sim(Y=Y,W=W,Xpop=Xpop,Xdow=Xdow,Xden=Xden,rho_temp = rho_temp, tau_temp =tau_temp,
                          tau_spat = tau_spat, rho_spa=rho_spat,
                          b_dow = b_dow,
                          b_time = b_time,
...
# sim_num=47
# sim_save_path = paste("R:\\Research problems\\JAGS\\Spatial temporal Bayesian hierarchical model\\outbreak_detection_marginalization(Latest)\\Favorite Baby!!\\simulated rds\\sim_",
# sim_num,sep='')
sim_save_path = paste("R:\\Research problems\\JAGS\\Spatial temporal Bayesian hierarchical model\\outbreak_detection_marginalization(Latest)\\Favorite Baby!!\\simulated rds\\REAL DATA",
                      sep='')
...
for(day_b in 1:day_last){
  ...
  Y_pred_mat_temp = apply(exp((pred_temp$EY_pred)),c(1,2),rpois,n=1)##  Y_pred
  EY_pred_temp = apply(Y_pred_mat_temp,2,mean) ## Y_pred
  ...
  crit_bool_temp = get_p_val(Y_next_true,Y_pred_mat_temp)>alpha_thres
  Y_pred_sampled0 = apply(Y_pred_mat_temp,2,sample,size=1)
  Y_pred_sampled = Y_pred_sampled0*(Y_pred_sampled0<Y_next_true)+(Y_pred_sampled0>=Y_next_true)*Y_next_true
  Y_update_vec = Y_next_true*crit_bool_temp + Y_pred_sampled*(1-crit_bool_temp) ## Adjusting for large p-value
