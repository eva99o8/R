...
#for (day_b in 99:109) {
  ...
  b_wkday_mean = next_prior[1,2];b_wkday_sd = next_prior[1,4]
  b_wkend_mean = next_prior[2,2];b_wkend_sd = next_prior[2,4]
  b_den_mean = next_prior[3,2];  b_den_sd = next_prior[3,4]
  b_trend_mean = next_prior[4,2];b_trend_sd = next_prior[4,4]
  tau_spat_alpha = 2+ (next_prior[7,2]/next_prior[7,4])^2; 
  tau_spat_beta = next_prior[7,2]+ (next_prior[7,2])^3/(next_prior[7,4])^2;
  tau_temp_alpha = (next_prior[8,2]/next_prior[8,4])^2;
  tau_temp_beta = next_prior[8,2]/(next_prior[8,4]^2)
#}
